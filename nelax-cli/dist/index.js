...
program.command("verify <code>").description("Verify the received OTP code and retrieve/deploy the Stellar wallet").option("--json", "Output result in JSON format").action(async (code, options) => {
  ...
  try {
    ...
    if (options.json) {
      ...
      console.log(
        chalk.whiteBright.bold("Explorer:               ") + chalk.underline.blue(`https://stellar.expert/explorer/testnet/account/${walletAddress}`)
      );
...
program.command("fund [address]").description("Request 10,000 testnet XLM from Stellar Friendbot to activate or top up a wallet").option("--json", "Output result in JSON format").action(async (addressArg, options) => {
  ...
  try {
    ...
    if (options?.json) {
      ...
      console.log(chalk.whiteBright.bold("Network:        ") + chalk.magenta("Stellar Testnet"));
      console.log(
        chalk.whiteBright.bold("Explorer:       ") + chalk.underline.blue(`https://stellar.expert/explorer/testnet/account/${targetAddress}`)
      );
      console.log(chalk.gray("\u2500".repeat(60)) + "\n");
    }
  } catch (err) {
