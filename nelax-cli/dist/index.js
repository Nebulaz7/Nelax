...
var NelaxPollarService = class {
  ...
  async verifyEmailOtp(code, clientSessionId) {
    ...
    const walletAddress = loginData?.content?.data?.providers?.wallet?.address || loginData?.content?.walletAddress || loginData?.content?.data?.wallet?.address || loginData?.content?.address;
    if (!walletAddress) {
...
program.name("nelax").description(chalk.cyanBright("\u26A1 Nelax \u2014 Autonomous AI Agent Wallets on Stellar (Pollar & x402)")).version("0.1.0", "-v, --version", "Output the current version of Nelax CLI");
...
program.command("verify <code>").description("Verify the received OTP code and retrieve/deploy the Stellar wallet").option("--json", "Output result in JSON format").action(async (code, options) => {
  ...
  try {
    ...
    if (options.json) {
      ...
      console.log(
        chalk.whiteBright.bold("Explorer:               ") + chalk.underline.blue(`https://testnet.stellar.expert/explorer/testnet/account/${walletAddress}`)
      );
...
program.command("rent <machineId>").description("Autonomously rent a compute node via the x402 payment protocol").option("-u, --url <url>", "Base URL of the Nelax compute marketplace", process.env.NELAX_MARKETPLACE_URL || "http://localhost:3000").option("--json", "Output result in JSON format").action(async (machineId, options) => {
  const session = getSession();
  if (!session || !session.walletAddress) {
    console.log(chalk.red("\nAuthentication required. Run nelax login <email> first.\n"));
    process.exit(1);
...
program.command("fetch <url>").description("Perform an autonomous x402 HTTP fetch (detects 402, auto-pays, retries with proof)").option("-X, --method <method>", "HTTP method (GET or POST)", "GET").option("--json", "Output result in JSON format").action(async (url, options) => {
  const session = getSession();
  if (!session || !session.walletAddress) {
    console.log(chalk.red("\nAuthentication required. Run nelax login <email> first.\n"));
    process.exit(1);
...
program.command("history").description("Display recent transaction history on Stellar Testnet for the active wallet").option("-l, --limit <number>", "Number of transactions to display", "10").option("--json", "Output result in JSON format").action(async (options) => {
  const session = getSession();
  if (!session || !session.walletAddress) {
    console.log(chalk.red("\nAuthentication required. Run nelax login <email> first.\n"));
    process.exit(1);
